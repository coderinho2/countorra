...
/**
 * REFERENCE COMPONENT — NOT WIRED INTO THE PRODUCT.
 *
 * Stored as a layout/interaction reference for the upcoming visual redesign
 * pass. It is imported by nothing: the live pricing page remains
 * `src/app/pricing/page.tsx`, which is the source of truth for what
 * Countorra actually charges.
 *
 * ⚠ EVERY NUMBER AND PLAN NAME BELOW IS PLACEHOLDER DEMO DATA from the
 * upstream 21st.dev component — "Developer / Production / Enterprise", edge
 * requests, $29/$39, $0.40 per 1M, "Save 20%", and the monthly/annual toggle
 * are all fictional. None of it describes Countorra, whose real plans
 * (Free / Premium / Business) and real AI message limits come from
 * `src/domain/billing/limits.ts` and the seeded rows in
 * `supabase/migrations/0010_billing.sql`. Do not copy any figure from this
 * file into the product.
 *
 * Deviation from the supplied source: the `"use client"` directive above was
 * added. The original has none, and it calls `useState` — under the App
 * Router it would throw at build time as a Server Component. That is a
        ...
        <div className="mb-16 flex flex-col items-center text-center">
          ...
          <div className="mt-10 flex items-center gap-3">
            ...
            <span className={cn("flex items-center gap-2 text-sm font-medium", isAnnual ? "text-white" : "text-neutral-500")}>
              Annually
              <span className="rounded-full bg-white/[0.08] px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest text-white">Save 20%</span>
            </span>
          </div>
        </div>
        ...
        <div className="grid w-full grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="flex flex-col rounded-xl border border-white/[0.08] bg-[#050505] p-8 transition-colors hover:border-white/[0.15]">
            ...
            <ul className="flex flex-col gap-4 text-sm text-neutral-300">
              {["100,000 edge requests/mo", "3 global edge regions", "Shared infrastructure", "Community support", "1 week log retention"].map(
                (feature, i) => (
          ...
          <div className="relative flex flex-col rounded-xl border border-white/[0.25] bg-black p-8 shadow-2xl">
            ...
            <div className="mb-6 flex items-start justify-between">
              ...
              <span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-black">Most Popular</span>
            </div>

            <div className="mb-8 flex items-baseline gap-1">
              <span className="text-4xl font-medium tracking-tighter">${isAnnual ? "29" : "39"}</span>
              <span className="text-sm font-medium text-neutral-500">/ user / mo</span>
            </div>
            ...
            <ul className="flex flex-col gap-4 text-sm text-neutral-300">
              {[
                "10,000,000 edge requests/mo",
                "150+ global edge regions",
...
/** Readable alias. `Component` is kept because the upstream demo imports that name. */
export const PricingSectionReference = Component;
