...
function FeatureCard({
  ...
  const subtitleClass = card.invert
    ? 'text-white/80'
  ...
  return (
    ...
    >
      {card.image && (
        // eslint-disable-next-line @next/next/no-img-element -- reference component ships a plain <img>; kept verbatim, and the demo sources are remote CDN URLs that next/image would need domain config for.
        <img
          src={card.image}
          alt={card.imageAlt ?? ''}
          decoding="async"
      ...
      <div className={cn('flex h-full flex-col items-start', vs.cardBody)}>
        ...
        <p className={cn('mt-1 text-sm', subtitleClass)}>
          {card.subtitle}
        </p>
